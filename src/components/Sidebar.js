...
export default function Sidebar({ isMobileMenuOpen, setIsMobileMenuOpen }) {
  ...
  React.useEffect(() => {
    async function fetchUserData() {
      try {
        // 1. AWS Amplify se current token nikaalo
        const session = await fetchAuthSession();
        const token = session.tokens?.idToken?.toString() || session.tokens?.accessToken?.toString();

        // 2. Token ko API call me bhejo
  ...
  return (
    <>
      ...
      <aside className={`dashboard-sidebar ${isCollapsed ? 'collapsed' : ''} ${isMobileMenuOpen ? 'mobile-open' : ''}`} style={{ width: isCollapsed ? '72px' : '260px', transition: isMounted ? 'width 0.3s cubic-bezier(0.4, 0, 0.2, 1)' : 'none', overflow: 'visible', flexShrink: 0 }}>
              ...
              <>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.5rem', background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444', fontSize: '0.875rem' }}
                  >
